...
RUN_ID="${1:-round3_$(date -u +%Y%m%d_%H%M%S)}"
SHARDS="${2:-8}"
CONFIG="${3:-configs/round3_core.yaml}"
MAX_QUESTIONS="${4:-0}"
MAX_TRIALS="${5:-0}"
...
for i in $(seq 0 $((SHARDS - 1))); do
  tmux kill-session -t "round3_s${i}" 2>/dev/null || true
  ...
  tmux new-session -d -s "round3_s${i}" \
  ...
  echo "launched round3 shard ${i}/${SHARDS}"
done
